...
let contact_rubber = (ball, rubber, a) => {
    ...
    // let relative_ball = cg.mMultiply(cg.mTranslate(ball.loc), cg.mScale(ball_r))
    let ball_rela = cg.mMultiply(change_coords, ball.getGlobalMatrix())
    // console.log("rubber",rubber.getGlobalMatrix())
    // console.log("change coords", change_coords)
    // console.log("ball_rela", ball_rela)
    // ball_rela = cg.mMultiply(cg.mTranslate(0, 0, -1), ball_rela)
...
export const init = async model => {
    ...
    paddle.move(0, 1, .1).turnX(-Math.PI / 2).turnY(Math.PI)
